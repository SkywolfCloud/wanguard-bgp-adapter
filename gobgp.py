...
def ip_zone_id():
    r = requests.get(
        Config.WANGUARD_BASE_URI + "/v1/ip_zones",
        params={"ip_zone_name": Config.WANGUARD_IP_ZONE},
        headers=header,
    )
...
def communities_init():
    with open("communities.yaml") as f:
        ...
        for i, j in communities["template"].items():
            r = requests.get(
                Config.WANGUARD_BASE_URI + "/v1/threshold_templates",
                params={"threshold_template_name": j},
                headers=header,
            )
            if r.status_code == 200:
                if i in ip_groups:
                    ip_groups_templates[i] = r.json()[0]["threshold_template_id"]
                else:
...
def add_prefix(prefix, ip_group, template):
    r = requests.get(
        Config.WANGUARD_BASE_URI + "/v1/ip_zones/" + str(IP_ZONE_ID) + "/prefixes",
...
def handle_route_added(route):
    ...
    for com in communities:
        ...
        elif com in templates:
            template = templates[com]
    if not template:
        for com in communities:
            if com in ip_groups_templates:
                template = ip_groups_templates[com]
...
def main():
    ...
    watch_request = gobgp_pb2.WatchEventRequest(
        table=gobgp_pb2.WatchEventRequest.Table(
            filters=[gobgp_pb2.WatchEventRequest.Table.Filter(init=True)]
        )
    )
    # 订阅表格事件
